<template>
	<view>
		<view class="header" v-bind:class="{'status':isH5Plus}">
			...
			<view class="setting"><image src="../../static/personalCenter/setting.png"></image></view>
		</view>
		<view class="orders">
			<view class="box">
				<view class="label" v-for="(row,index) in orderTypeLise" wx:key="row.name" hover-class="hover" hover-stay-time="50" @tap="toOrderType(index)">
					<view class="icon"><view class="badge" v-if="row.badge>0">{{row.badge}}</view><image :src="'../../static/personalCenter/'+row.icon"></image></view>
					{{row.name}}
				</view>
			</view>
		</view> 
